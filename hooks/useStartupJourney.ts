...
export const useStartupJourney = (projectId: string | null, locale: Locale) => {
    ...
    const advanceStage = async (currentStage: Stage, currentData: Partial<StartupData>, currentMessages: ChatMessage[]) => {
        ...
        if (!nextStage) {
            ...
            await saveProjectToDb(Stage.COMPLETE, currentData, currentMessages);
            return;
        }
        ...
        if (isSummaryStage(nextStage)) {
            setIsReadyForNextSection(true);
             await saveProjectToDb(nextStage, currentData, currentMessages);
            ...
            await handleAutoGeneration(nextStage, currentData, [...currentMessages, { id: uuidv4(), text: question, sender: 'ai' }]);
        } else {
    ...
    const handleSendMessage = async (messageText: string) => {
     ...
     const handleSuggestionAccept = async (messageId: string, acceptedAnswer: string) => {
        setIsLoading(true);
        setMessages(prev => prev.filter(m => m.id !== messageId));
        await handleSendMessage(acceptedAnswer);
    };
    ...
    const handleRequestSuggestion = async () => {
        ...
        try {
            ...
            addMessage({ text: suggestion, sender: 'ai', isSuggestion: true });
        } catch(e) {
            console.error(e);
    ...
    const handleRefineSuggestion = async (messageId: string, originalSuggestion: string, instruction: string) => {
        setIsLoading(true);
        addMessage({ text: `${t('system_refining_suggestion', locale)}`, sender: 'system'});
        try {
            ...
            setMessages(prev => prev.map(m => m.id === messageId ? { ...m, text: refinedText } : m).filter(m => m.sender !== 'system'));
        } catch (error) {
