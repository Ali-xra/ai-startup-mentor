      ...
      {isOpen && (
        <div className={`absolute mt-2 w-56 ${locale === 'fa' ? 'left-0 origin-top-left' : 'right-0 origin-top-right'} bg-white dark:bg-slate-800 rounded-md shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none z-50`}>
          <div className="py-1">
            ...
            <button
              onClick={() => { onExportWord(); setIsOpen(false); }}
              className={`w-full flex items-center gap-3 px-4 py-2 text-sm text-slate-700 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-700 ${locale === 'fa' ? 'text-right flex-row-reverse' : 'text-left'}`}
            >
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5"><path strokeLinecap="round" strokeLinejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 0 0-3.375-3.375h-1.5A1.125 1.125 0 0 1 13.5 7.125v-1.5a3.375 3.375 0 0 0-3.375-3.375H8.25m2.25 0v3m0 0v3.75m0-3.75h9.75c.621 0 1.125.504 1.125 1.125v11.25c0 .621-.504 1.125-1.125 1.125H8.25c-.621 0-1.125-.504-1.125-1.125V11.25c0-.621.504-1.125 1.125-1.125H9m6.75 0v3.75M9 15.75v3.75M15 15.75v3.75" /></svg>
            ...
            <button
              ...
              className={`w-full flex items-center gap-3 px-4 py-2 text-sm text-slate-700 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-700 ${locale === 'fa' ? 'text-right flex-row-reverse' : 'text-left'}`}
            >
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5"><path strokeLinecap="round" strokeLinejoin="round" d="M3.375 19.5h17.25m0 0a1.125 1.125 0 0 1-1.125 1.125H4.5a1.125 1.125 0 0 1-1.125-1.125m0 0V5.25a1.125 1.125 0 0 1 1.125-1.125h4.5a1.125 1.125 0 0 1 1.125 1.125v13.5m-6.75 0h6.75m0 0v-3.375A3.375 3.375 0 0 0 12.375 15H8.25" /></svg>
            ...
            >
              {currentTheme === 'dark' ? (
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5">
                  <path strokeLinecap="round" strokeLinejoin="round" d="M12 3v2.25m6.364.386-1.591 1.591M21 12h-2.25m-.386 6.364-1.591-1.591M12 18.75V21m-6.364-.386 1.591-1.591M3 12h2.25m.386-6.364 1.591 1.591M12 8.25a3.75 3.75 0 1 0 0 7.5 3.75 3.75 0 0 0 0-7.5Z" />
                </svg>
              ) : (
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5">
              ...
              <span>{currentTheme === 'dark' ? (t('theme_light', locale) || 'روشن') : (t('theme_dark', locale) || 'تیره')}</span>
            </button>
            <button
