...
export const PlanBadge: React.FC<PlanBadgeProps> = ({ locale }) => {
    ...
    const planColors: Record<string, { bg: string; text: string; border: string }> = {
        Free: { bg: 'bg-slate-100 dark:bg-slate-800', text: 'text-slate-700 dark:text-slate-300', border: 'border-slate-300 dark:border-slate-600' },
        Starter: { bg: 'bg-blue-100 dark:bg-blue-900/30', text: 'text-blue-700 dark:text-blue-300', border: 'border-blue-300 dark:border-blue-700' },
        Pro: { bg: 'bg-purple-100 dark:bg-purple-900/30', text: 'text-purple-700 dark:text-purple-300', border: 'border-purple-300 dark:border-purple-700' },
        Enterprise: { bg: 'bg-yellow-100 dark:bg-yellow-900/30', text: 'text-yellow-700 dark:text-yellow-300', border: 'border-yellow-300 dark:border-yellow-700' }
    };
        ...
        <div className={`${colors.bg} ${colors.text} border ${colors.border} rounded-lg p-3 shadow-sm`}>
            ...
            <div className="space-y-1 text-sm">
                ...
                <div className="flex items-center justify-between">
                    ...
                    <span className="font-semibold">
                        {maxTeamMembers === Infinity ? (locale === 'fa' ? 'نامحدود' : 'Unlimited') : maxTeamMembers}
                    </span>
                </div>

                <div className="flex items-center justify-between">
                    <span className="opacity-75">
                        {locale === 'fa' ? '🎯 مراحل:' : '🎯 Phases:'}
                    </span>
                    <span className="font-semibold">
