...
export const ProjectSelectionScreen: React.FC<ProjectSelectionScreenProps> = ({
  ...
  const [selectedProjectForMembers, setSelectedProjectForMembers] = useState<ProjectSummary | null>(
    null
  );
 ...
 `
      ...
      const invitedByIds = [
        ...new Set((memberProjects || []).map((mp) => mp.invited_by).filter(Boolean)),
      ];
      ...
      const sharedProjects = (memberProjects || []).map((mp) => mp.projects).filter(Boolean);
      ...
      const uniqueProjects = allProjects.filter(
        (project: any, index, self) => index === self.findIndex((p: any) => p.id === project.id)
      );
      ...
      const projectSummaries: ProjectSummary[] = uniqueProjects.map((p: any) => {
        ...
        const displayName = memberData?.user_email || memberData?.invited_by || 'نامشخص';
  ...
  const handleCreateProject = async (e: React.FormEvent) => {
    ...
    if (error || !data) {
      ...
      alert(
        `Error creating project: ${error?.message || 'Could not retrieve project after creation.'}`
      );
    } else {
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      if (!data || data.length === 0) {
        const rlsErrorMsg =
          "Deletion failed. This is likely due to a security policy (RLS) on your database. Please ensure your 'DELETE' policy in the Supabase dashboard allows users to delete their own records (e.g., using `auth.uid() = user_id`).";
        alert(rlsErrorMsg);
      ...
      <div className="min-h-screen bg-slate-50 dark:bg-slate-900 flex items-center justify-center p-4 font-sans transition-colors duration-300">
        <div className="w-full max-w-4xl mx-auto relative">
          <div
            ...
            style={{ [locale === 'fa' ? 'left' : 'right']: '1rem' }}
          >
            <LanguageSelector />
            <button
              onClick={signOut}
            ...
            <div className="mb-6 p-4 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg">
              ...
              <div className="space-y-3">
                {pendingInvitations.map((invitation) => (
                  <div
                    key={invitation.id}
                    className="flex items-center justify-between p-3 bg-white dark:bg-slate-800 rounded-lg border border-blue-200 dark:border-blue-800"
                  >
                    <div>
                ...
                <div className="space-y-3">
                  {projects.map((p) => {
                    ...
                    const displayName = isShared
                      ? `${p.name} - به اشتراک گذاشته شده توسط ${p.shared_by}`
                      : p.name;

                    return (
                      <div
                        key={p.id}
                        className="group flex flex-col gap-2 p-3 bg-slate-100/50 dark:bg-slate-900/50 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-900 transition-all"
                      >
                        <div className="flex items-center justify-between">
                          <div
