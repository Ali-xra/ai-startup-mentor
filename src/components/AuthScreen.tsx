...
export const AuthScreen: React.FC<AuthScreenProps> = ({ locale }) => {
            ...
            if (isLogin) {
                ...
                if (authData.user) {
                    ...
                    const { data: profile } = await supabase
                        ...
                        .maybeSingle(); // تغییر از .single() به .maybeSingle()
                    ...
                    if (profile?.role && rolePages[profile.role]) {
                        window.location.href = rolePages[profile.role];
                    } else {
                        // اگه profile نداره یا role نامعتبر داره، به login.html بره برای انتخاب نقش
                        window.location.href = '/login.html';
                    }
                }
            } else {
                // Signup - فقط ساخت حساب (بدون role)
                ...
                if (authData.user) {
                    // بعد از signup موفق، به login.html هدایت می‌شه
                    // و اونجا چک می‌کنه که profile داره یا نه و باید role انتخاب کنه
                    window.location.href = '/login.html';
                }
            }
        } catch (err: any) {
    ...
    const handleGoogleLogin = async () => {
        ...
        // OAuth flow - بعد از login بر می‌گرده به /login.html
        ...
        // بعد از redirect، AppContent چک می‌کنه که profile داره یا نه
        // اگه نداره، RoleSelection رو نمایش میده
    };
