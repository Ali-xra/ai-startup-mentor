...
export const UpgradeRequestsPanel: React.FC<UpgradeRequestsPanelProps> = ({ adminId }) => {
    ...
    const handleApprove = async (requestId: string, durationMonths: number = 1) => {
        if (!confirm(`Are you sure you want to approve this request for ${durationMonths} month(s)?`)) return;
    ...
    const getStatusBadge = (status: UpgradeRequestStatus) => {
        const badges = {
            [UpgradeRequestStatus.PENDING]: 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-300',
            [UpgradeRequestStatus.APPROVED]: 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300',
            [UpgradeRequestStatus.REJECTED]: 'bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-300',
            [UpgradeRequestStatus.EXPIRED]: 'bg-gray-100 text-gray-800 dark:bg-gray-900/30 dark:text-gray-300'
        };
    ...
    return (
        <div className="p-6">
            ...
            <div className="flex gap-2 mb-6">
                {['all', UpgradeRequestStatus.PENDING, UpgradeRequestStatus.APPROVED, UpgradeRequestStatus.REJECTED, UpgradeRequestStatus.EXPIRED].map((f) => (
                    <button
                        key={f}
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap text-sm">
                                            {request.expires_at ? (
                                                <div>
                                                    ...
                                                    {daysLeft !== null && (
                                                        <div className={`text-xs ${daysLeft < 7 ? 'text-red-600 dark:text-red-400' : 'text-slate-500 dark:text-slate-400'}`}>
            ...
            {requests.some(r => r.admin_notes) && (
                <div className="mt-6 p-4 bg-slate-100 dark:bg-slate-800 rounded-lg">
                    <h3 className="text-sm font-semibold mb-2 text-slate-900 dark:text-white">Admin Notes:</h3>
                    {requests
