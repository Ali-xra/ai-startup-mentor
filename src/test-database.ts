...
async function testDatabaseSetup() {
  ...
  try {
    ...
    const { data: rpcProjects, error: rpcError } = await supabase.rpc('get_public_projects', {
      p_filter: 'all',
      p_limit: 5,
      p_offset: 0,
    });
    ...
    const { data: hasLiked, error: hasLikedError } = await supabase.rpc('has_user_liked_project', {
