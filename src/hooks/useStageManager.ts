...
export const isSummaryStage = (stage: Stage) => stage.endsWith('_SUMMARY') || stage === Stage.COMPLETE;
...
interface UseStageManagerProps {
    locale: Locale;
    onSaveProject: (stage: Stage, data: Partial<StartupData>, messages: ChatMessage[]) => Promise<void>;
...
export const useStageManager = ({
     ...
     */
    const jumpToStage = useCallback((selectedStage: Stage, currentStage: Stage): Stage | null => {
        const currentIndex = ALL_STAGES.indexOf(currentStage);
