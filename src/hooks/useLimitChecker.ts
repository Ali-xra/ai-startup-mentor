...
export const useLimitChecker = () => {
  ...
  const checkProjectLimit = useCallback(
    (currentProjectsCount: number): LimitCheckResult => {
   ...
   */
  const checkAIMessageLimit = useCallback(
    (currentAIMessagesUsed: number): LimitCheckResult => {
   ...
   * چک کردن محدودیت فاز
   */
  const checkPhaseLimit = useCallback(
    (requestedPhase: number): LimitCheckResult => {
   ...
   */
  const checkTeamMemberLimit = useCallback(
    (currentTeamMembersCount: number): LimitCheckResult => {
  ...
  const showLimitReachedModal = useCallback(
    (limitType: LimitType, currentValue?: number, maxValue?: number) => {
   ...
   */
  const checkAndShowLimit = useCallback(
