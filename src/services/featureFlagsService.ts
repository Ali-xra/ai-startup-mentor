...
export const featureFlagsService = {
     ...
     */
    async getAllUsersWithFeatures(limit: number = 50, offset: number = 0): Promise<UserWithFeatures[]> {
        try {
    ...
    async toggleUserFeature(
        ...
        try {
            ...
            await this.logAdminAction(
                adminId,
                isEnabled ? 'grant_feature' : 'revoke_feature',
                userId,
                { feature_key: featureKey, notes }
            );
        } catch (error) {
