...
const callGeminiDirect = async (payload: any): Promise<any> => {
  ...
  const sources =
    result?.candidates?.[0]?.groundingMetadata?.groundingChunks?.map((chunk: any) => ({
      uri: chunk.web?.uri || '',
...
const buildPromptFromConfig = (
  ...
  let systemInstruction =
    config.role ||
    `You are an AI startup mentor helping entrepreneurs develop their business ideas.`;
...
export const refineText = async (
  ...
  const payload = buildPrompt(
    'refineText',
    { originalText, instruction, startupData, locale },
    locale
  );
...
export const generateSectionSummary = async (
  ...
  const payload = buildPrompt(
    ...
    { summaryStage, startupData, locale },
    locale
  );
...
export const generateImageIdeas = async (
  ...
  startupData: Partial<StartupData>,
  locale: Locale
): Promise<{ text: string; jsonData: string; images: string[] }> => {
  const payload = buildPrompt(
    'generateImageIdeas',
    { stage, userInput, startupData, locale },
    locale
  );
